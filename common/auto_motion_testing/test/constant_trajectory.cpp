...
TEST(ConstantTrajectory, Stationary)
{
  const auto x0 = 3.0F;
  const auto y0 = -5.0F;
  const auto traj = autoware::motion::motion_testing::constant_velocity_trajectory(
...
TEST(ConstantTrajectory, ConstantAcceleration)
{
  ...
  EXPECT_FLOAT_EQ(std::cos(heading), c);
  const auto dt = std::chrono::milliseconds(100LL);
  const auto traj = autoware::motion::motion_testing::constant_acceleration_trajectory(
