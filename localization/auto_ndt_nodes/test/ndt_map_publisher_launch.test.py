...
def generate_test_description():
    ...
    map_param_file = \
        os.path.join(
            get_package_share_directory('auto_ndt_nodes'),
            'param/test_map_publisher.param.yaml')
    map_yaml_file_param = \
        os.path.join(get_package_share_directory('auto_ndt_nodes'), 'data/test_map.yaml')
    map_pcd_file_param = \
        os.path.join(get_package_share_directory('auto_ndt_nodes'), 'data/test_map.pcd')

    # map_provide node execution definition
    map_provider_node_runner = Node(
        package="auto_ndt_nodes",
